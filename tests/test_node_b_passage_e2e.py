...
from src.nodes import node_b
from src.nodes.node_b import (
    B_PASSAGE_MIN_QUALITY,
...
class TemporaryMain:
    """SQLite/MQTT-shaped harness for the B -> Main -> D transaction."""
    ...
    def on_b_passage(self, payload: dict) -> bool:
        ...
        event_key = f'B_PASSAGE:{payload["journey_id"]}'
        with self.db:
            ...
            self.db.execute(
                ...
                ("WAITING_D", json.dumps(["A", "B"]), payload["journey_id"]),
            )
...
class NodeBPassageE2ETest(unittest.TestCase):
    def setUp(self) -> None:
        self.a_embedding = unit(0)
        self.b_embeddings = [unit(0, 0.05), unit(0, -0.04)]
        self.samples = [
        ...
        self.payload = build_passage_payload(
            ...
            b_embeddings=self.b_embeddings,
            a_local_track_id=7,
            b_local_track_id=31,
            b_passage_timestamp="2026-08-14T09:02:00+09:00",
            selected_wire_samples=[
                sample for sample in self.samples if sample["gallery_selected"]
            ],
        )

    def test_payload_uses_real_samples_and_both_b_gallery_entries(self) -> None:
        expected_scores = [
            float(np.dot(embedding, self.a_embedding))
            for embedding in self.b_embeddings
        ]
        ...
        b_gallery = [item for item in self.payload["gallery"] if item["node_id"] == "B"]
        self.assertEqual(len(b_gallery), 2)
        self.assertEqual([item["quality"] for item in b_gallery], [0.86, 0.84])
    ...
    def test_operational_low_quality_journeys_are_held_locally(self) -> None:
        ...
        for journey_id, (score, quality) in cases.items():
            gallery = [
                ...
                {"node_id": "B", "embedding": unit(0).tolist(), "quality": quality},
                {"node_id": "B", "embedding": unit(0).tolist(), "quality": quality},
            ]
            accepted, reason, diagnostics = validate_b_passage_evidence(gallery)
    ...
    def test_low_quality_is_discarded_and_later_frames_can_fill_gallery(self) -> None:
        ...
        self.assertFalse(
            try_add_gallery(31, unit(1), B_PASSAGE_MIN_QUALITY - 0.01, 10,
    ...
    def test_j000053_demo_quality_distribution_selects_at_least_two(self) -> None:
        ...
        self.assertTrue(all(
            sample["quality"] >= B_PASSAGE_MIN_QUALITY
    ...
    def test_moving_person_temporal_windows_replace_and_publish_once(self) -> None:
        ...
        with patch.object(node_b, "candidate_reference", return_value=reference), \
             patch.object(node_b, "save_match_capture", return_value="capture.jpg"), \
             patch.object(node_b, "append_jsonl") as append_jsonl, \
             patch.object(node_b, "append_csv"), \
             patch.object(node_b, "mark_passed"), \
             patch.object(node_b, "log_revisit_event"), \
            ...
            for window_index, window_score in enumerate(similarity_windows, start=1):
                ...
                published = node_b.publish_passage(
                    client, 59, "J-MOVING",
    ...
    def test_temporal_candidate_bank_is_bounded(self) -> None:
        ...
        bank: list[dict] = []
        frame_index = 0
        for window_index in range(node_b.TEMPORAL_CANDIDATE_BANK_MAX + 3):
            for _ in range(node_b.TEMPORAL_WINDOW_SIZE):
                frame_index += 1
                add_temporal_candidate(
                    unit(0, 0.01 * (window_index + 1)), 0.75, frame_index,
                    incoming, window, bank,
                )
        self.assertEqual(len(bank), node_b.TEMPORAL_CANDIDATE_BANK_MAX)

    def test_all_main_contract_thresholds_are_required_locally(self) -> None:
        base = [
            {"node_id": "A", "embedding": unit(0).tolist(), "quality": 0.9},
            {"node_id": "B", "embedding": unit(0, 0.05).tolist(), "quality": 0.81},
            {"node_id": "B", "embedding": unit(0, -0.04).tolist(), "quality": 0.82},
        ]
        accepted, reason, diagnostics = validate_b_passage_evidence(base)
    ...
    def test_j000050_actual_wire_gallery_is_rejected_with_main_scores(self) -> None:
        ...
        accepted, reason, _ = validate_b_passage_evidence(fixture["gallery"])
        self.assertFalse(accepted)
    ...
    def test_j000050_is_rejected_by_actual_publish_path(self) -> None:
        ...
        b_gallery = [item for item in fixture["gallery"] if item["node_id"] == "B"]
        b_embeddings = [np.asarray(item["embedding"], dtype=np.float32) for item in b_gallery]
        selected = [
            {"frame_index": index, "best_score": score,
             "quality": item["quality"], "gallery_selected": True}
            for index, (score, item) in enumerate(zip(
                [0.6805351972579956, 0.65985107421875], b_gallery
            ))
        ]
        observed = [
        ...
        with patch.object(node_b, "candidate_reference", return_value=reference), \
             ...
             patch.object(node_b, "log_revisit_event"):
            published = node_b.publish_passage(
                client, 77, "J000050", b_embeddings,
    ...
    def test_positive_wire_fixture_and_roundtrip_invariants(self) -> None:
        ...
        accepted, reason, _ = validate_b_passage_evidence(self.payload["gallery"])
        self.assertTrue(accepted, reason)
    ...
    def test_positive_actual_publish_path_excludes_unselected_observation(self) -> None:
        ...
        with patch.object(node_b, "candidate_reference", return_value=reference), \
             patch.object(node_b, "save_match_capture", return_value="capture.jpg"), \
             patch.object(node_b, "append_jsonl"), \
             patch.object(node_b, "append_csv"), \
             patch.object(node_b, "mark_passed"), \
             patch.object(node_b, "log_revisit_event"):
            published = node_b.publish_passage(
                client, 31, "J-E2E-1", self.b_embeddings,
    ...
    def test_revisit_log_is_structured_and_contains_no_raw_embedding(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            log_path = Path(temp_dir) / "camera_b_revisit.jsonl"
            payload = {
                "event": "CANDIDATE",
            ...
            with patch.object(node_b, "REVISIT_LOG", log_path), \
                 patch.object(node_b, "REVISIT_RUN_ID", "RUN-LOG-1"), \
                 patch.dict(node_b.candidates, {}, clear=True), \
                 patch.object(node_b, "append_csv"):
                node_b.save_candidate(payload)
                node_b.pending_passage_pubacks[77] = {
                    "request_id": "REQ-1", "journey_id": "J-LOG-1",
                    "person_uid": "P-LOG-1", "local_track_id": 7,
                    "topic": node_b.PASSAGE_TOPIC, "rc": 0,
                }
                node_b.on_publish(None, None, 77, 0, None)

            lines = log_path.read_text(encoding="utf-8").splitlines()
            records = [json.loads(line) for line in lines]
            self.assertEqual(
                [record["event"] for record in records],
                ["B_CANDIDATE_RECEIVED", "B_CANDIDATE_ACTIVATED", "B_PASSAGE_PUBLISH"],
            )
            received = records[0]
    ...
    def test_main_transition_single_d_publish_and_duplicate_idempotency(self) -> None:
        main = TemporaryMain()
        self.assertTrue(main.on_b_passage(self.payload))
        self.assertTrue(main.on_b_passage(self.payload))
    ...
    def test_temporary_mqtt_and_sqlite_roundtrip(self) -> None:
        ...
        try:
            ...
            main_client.subscribe("cctv/events/b/passage", qos=1)
            observer.subscribe("cctv/candidates/d", qos=1)
            main_client.loop_start()
            observer.loop_start()
            publisher.loop_start()
            encoded = json.dumps(self.payload)
            publisher.publish("cctv/events/b/passage", encoded, qos=1).wait_for_publish()
            publisher.publish("cctv/events/b/passage", encoded, qos=1).wait_for_publish()
    ...
    def test_confirmation_observations_reuse_enables_passage_on_normal_short_walk(self) -> None:
        ...
        b_embeddings = [item["embedding"] for item in selected]
        ...
        with patch.object(node_b, "candidate_reference", return_value=reference), \
             ...
             patch.object(node_b, "log_revisit_event"):
            published = node_b.publish_passage(
                client, 42, candidate_id, b_embeddings,
    ...
    def test_baseline_j000010_strong_match_maintains_passage_success(self) -> None:
        ...
        gallery.extend([
            {"node_id": "B", "embedding": c_emb1.tolist(), "quality": 0.949259},
            {"node_id": "B", "embedding": c_emb2.tolist(), "quality": 0.904701},
        ])
    ...
    def test_stranger_and_wrong_identity_never_promoted_or_published(self) -> None:
        ...
        gallery = [
            ...
            {"node_id": "B", "embedding": c_low.tolist(), "quality": 0.85},
            {"node_id": "B", "embedding": c_low.tolist(), "quality": 0.85},
        ]
        accepted, reason, _ = validate_b_passage_evidence(gallery)
        self.assertFalse(accepted)
    ...
    def test_partial_temporal_window_finalization_on_track_lost(self) -> None:
        ...
        selected = selected_temporal_candidates(bank)
        self.assertEqual(len(selected), 2)
        self.assertTrue(all(item["quality"] >= B_PASSAGE_MIN_QUALITY for item in selected))
    ...
    def test_insufficient_evidence_only_one_sample_rejects_passage(self) -> None:
        """Case F: If only 1 valid observation exists, validate_b_passage_evidence rejects and publish_passage fails."""
        incoming = [{
            "node_id": "A", "captured_at": "2026-08-15T09:00:00+09:00",
            "embedding_dim": 512, "embedding": unit(0).tolist(), "quality": 0.90,
        }]
        ...
        single_gallery = [
            {"node_id": "A", "embedding": unit(0).tolist(), "quality": 0.90},
            {"node_id": "B", "embedding": unit(0, 0.01).tolist(), "quality": 0.85},
        ]
        accepted, reason, diagnostics = validate_b_passage_evidence(single_gallery)
        self.assertFalse(accepted)
        self.assertEqual(reason, "INSUFFICIENT_QUALITY")
        self.assertIsNone(diagnostics)
